...
FifoChannel::FifoChannel(std::string path) : m_path(std::move(path)) {}
...
bool FifoChannel::openWriter()
{
    ...
    if (m_writerFd < 0) { ::perror("FifoChannel::openWriter"); return false; }
    return true;
}
...
{
    if (m_writerFd >= 0) { ::close(m_writerFd); m_writerFd = -1; }
}
...
bool FifoChannel::openReader()
{
    ...
    if (m_readerFd < 0) { ::perror("FifoChannel::openReader"); return false; }
    return true;
}
...
{
    if (m_readerFd >= 0) { ::close(m_readerFd); m_readerFd = -1; }
}
