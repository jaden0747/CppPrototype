...
std::unique_ptr<ObjMesh> ObjMesh::load(const char* fileName, bool center, bool genTangents)
{
    ...
    cout << "Loaded mesh from: " << fileName << " vertices = " << (glMesh.points.size() / 3)
...
ObjMesh::ObjMeshData::ObjVertex::ObjVertex(std::string& vertString, ObjMeshData* mesh)
    : pIdx(-1)
    , nIdx(-1)
    , tcIdx(-1)
{
    ...
    if (pIdx < 0)
        pIdx += static_cast<int>(mesh->points.size());
    else
        pIdx--;
    if (slash1 != string::npos)
    {
        ...
        if (slash2 > slash1 + 1)
        {
            ...
            if (tcIdx < 0)
                tcIdx += static_cast<int>(mesh->texCoords.size());
            else
                tcIdx--;
        }
        nIdx = std::stoi(vertString.substr(slash2 + 1));
        if (nIdx < 0)
            nIdx += static_cast<int>(mesh->normals.size());
        else
            nIdx--;
    }
}
