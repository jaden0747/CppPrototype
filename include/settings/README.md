...
Declare `SettingsItem<T>` at file scope. Registration happens at static-init time — before `main()` runs:
...
SettingsItem<AppConfig>      g_app("AppConfig");       // key = "AppConfig"
SettingsItem<RenderSettings> g_render("RenderSettings");
```

The string key is how you address the item via the registry and the JSON file.
