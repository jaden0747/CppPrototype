...
//   // fill *slot ...
//   sender.reverse();   // optional — byte-reverses *slot (requires trivially copyable T)
...
public:
    ...
    // Reverse the byte representation of the reserved T in-place.
    // Only callable when T is trivially copyable (checked at call site).
    void reverse()
    {
        static_assert(
            std::is_trivially_copyable<T>::value, "SenderPort<T>::reverse() requires T to be trivially copyable");
        std::lock_guard<std::mutex> lock(m_mu);
        if (!m_reserved)
            return;
        auto* bytes = reinterpret_cast<uint8_t*>(&m_reserved->data);
